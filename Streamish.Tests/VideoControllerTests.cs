...
namespace Streamish.Tests
{
    public class VideoControllerTests
    {
        ...
        [Fact]
        public void Search_Returns_All_MatchingVideos()
        {
            //Arrange
            var videoCount = 5;
            var videos = CreateTestVideos(videoCount);
            videos[0].Title = "Gilmore";
            videos[1].Title = "Happy Gilmore";
            ...
            //Act
            var result = controller.Search("Gilmore", false);

            //Assert
            var okResult = Assert.IsType<OkObjectResult>(result);
            var actualVideos = Assert.IsType<List<Video>>(okResult.Value);

            Assert.Equal(GilmoreCount, actualVideos.Count);
        }
        ...
        public void Get_Returns_All_Videos()
        {
            // Arrange 
            var videoCount = 20;
            var videos = CreateTestVideos(videoCount); //makes 20 videos
